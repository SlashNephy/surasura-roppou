...
まだフロントエンド基盤の初期段階です。
実装済みの主な要素は Vite + React + TanStack Router のアプリシェル、主要ページのプレースホルダー、shadcn/ui 系の共通 UI、PWA manifest、Vitest による基礎テストです。

プロダクトの詳細仕様は `docs/design-doc.md` を先に確認してください。
条文データ、OCR、オフライン保存、復習スケジューリング、BFF などは設計上の予定を含むため、実装済みと誤認しないでください。
