...
| OCR モデルとスキャナーのプライバシー         | 同意前の取得または意図しない保存と送信が発生する | [品質基準](./quality-baseline.md)の第 5 節、第 6 節、第 8.4 節に従い、Network と storage を段階別に監査する         |
| PWA storage とローカルデータ制御             | 保存データが失われる、または利用者が削除できない | [品質基準](./quality-baseline.md)の第 7 節と第 8.4 節に従い、export と全削除を監査する                              |
