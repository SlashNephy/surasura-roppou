...
表示モード切替から次の描画までを 3 回測定し、中央値が 200 ms を超えた場合は未達と判定する。

目次操作から次の描画までを 3 回測定し、中央値が 200 ms を超えた場合は未達と判定する。
...
| 表示モード切替             | 未検証 | 3 回の値、中央値、個別の判定、performance trace               |
| 目次操作                   | 未検証 | 3 回の値、中央値、個別の判定、performance trace               |
...
- [ ] 民法（lawId `129AC0000000089`）の実レスポンスを取得し、受信完了から最初の条文表示までを 3 回測定した。
- [ ] 表示モード切替と目次操作を別々に 3 回ずつ測定し、それぞれの中央値と判定を記録した。
...
5. 受信完了から最初の条文表示までは 3 回測定し、表示モード切替と目次操作は別々に 3 回ずつ測定して、各試行値、各中央値、個別の判定、Long Task、LCP、INP、CLS、console error を測定条件と対応付けて記録する。
