...
`src/app/pages.tsx` の `StudyPage` を次の実装に差し替える（`cardCount` の useState / useEffect を含めて置き換え。ファイル先頭の import は既存のまま使える）:
...
export const StudyPage = ({
  ...
  // undefined = 読み込み中、"error" = 読み込み失敗。
  const [counts, setCounts] = useState<StudyCounts | "error">();
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(([dueCards, unscheduledCards, cards]) => {
        if (isCurrent) {
          setCounts({
            due: dueCards.length,
        ...
        <p className="mt-1 text-xs opacity-75">
          {counts === undefined
            ...
            : counts === "error"
              ...
              : counts.due === 0
                ? "今日の復習はありません"
                : `${String(counts.due)} 件のカードが復習期限です`}
        </p>
        {typeof counts === "object" && counts.due > 0 ? (
          <Link
        ...
        <section className="rounded-md border bg-card p-4">
          ...
          <p className="mt-2 text-xs leading-5 text-muted-foreground">
            {typeof counts === "object"
              ? `${String(counts.unscheduled)} 件の未学習カード`
              : "未学習のカードから新しく覚えます"}
          </p>
          {typeof counts === "object" && counts.unscheduled > 0 ? (
            <Link
...
注意: 既存テスト `"shows the study card count and list link on the study page"` は `1 件のカード` と `カード一覧を開く` を期待している。上記実装の「条文カード」セクション（`counts.total`）がそのまま満たすので、既存テストは変更しない。
