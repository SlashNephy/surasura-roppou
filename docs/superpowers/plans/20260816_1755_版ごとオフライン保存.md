...
**Goal:** `savedLaws` を法令ごと 1 版から版ごとのレコードに変え、「現行版スロット」の概念を導入する。

完了条件は次の 2 つに分ける。

- **互換性（回帰させない）**: 手動保存と現行版フォールバックの経路・表示は従来どおり。`src/app/law-viewer-loader.ts` をはじめとする呼び出し側は無改修で通る。
- **意図した変更（回帰ではない）**: `savedAt` が法令単位から版単位になることに伴い、保存日時の表示・保存一覧の並び・エクスポート JSON の `savedAt` は変わる。詳細は設計の「`savedAt` の意味の変更と、それに伴う振る舞いの変化」を参照。
...
interface LegacySavedLawRecord {
  // 複合キーの構成要素は移行時に検証するため unknown で受ける。
  lawId: unknown;
  revisionId: unknown;
  nodeCount: number;
  savedAt: string;
...
export const migrateRecordsToVersion4 = async (
  ...
  // 下の catch で abort するのは、この検証を抜けたうえでの想定外の失敗だけである。
...
`src/core/storage/import-saved-data.ts` の for ループ内を差し替える。インポートは現行版スロットの入れ替えとして扱い、`saveLawDocument` と対称に、旧現行版はレコードもノードも残したまま `isCurrent: 0` へ降格する。インポートは他の法令を消さない以上「丸ごと復元」の意味論ではなく、ローカルの本文を破棄する理由がないため。

降格とノード削除は `saveLawDocument` と共通のヘルパー（`src/core/storage/current-revision-slot.ts`）に切り出し、不変条件を 1 箇所に閉じ込める。
...
for (const savedLaw of data.savedLaws) {
  ...
  // インポート対象の版のノードは常に一度消してから入れ直す。これを怠ると、その版が既に
  // 履歴版へ降格済みの場合にローカルのノードが残留し nodeCount と実件数がずれる。
...
期待: 型エラーなし。既存の `repository.test.ts` / `migrations.test.ts` / アプリ層のテストがすべて PASS。既存テストは互換性（手動保存と現行版フォールバック）の回帰スイートとして機能する。

ただし `savedAt` の版単位化は意図した変更なので、保存日時に依存するテストの期待値が変わることは回帰ではない。差分が出たら、互換性の破れか `savedAt` 由来かを切り分けてから直す。
