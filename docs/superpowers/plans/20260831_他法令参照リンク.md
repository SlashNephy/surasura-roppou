...
既存の `isRelativeParagraphShift` を項に限定しない名前へ広げるのではなく、条にも使える述語を追加する。`isRelativeParagraphShift` の定義の直後に置く。

```ts
// 条・項に共通の相対シフト（前条・次条・前項・次項）かどうか。
const isRelativeShift = (value: string): boolean => value === "previous" || value === "next";
```

`isRelativeParagraphShift` は既存の呼び出し箇所をそのまま残し、本体だけ `isRelativeShift` へ委譲する。

```ts
const isRelativeParagraphShift = (paragraph: string): boolean => isRelativeShift(paragraph);
```
