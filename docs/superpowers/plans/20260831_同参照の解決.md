...
- Consumes: なし
- Produces: `bodyReferencePositionPatternSource` が `同法` `同条` `同項` `同号` で始まるスパンにマッチするようになる。`referencePositionPatternSource`（OCR 用）は変わらない
...
Expected: 全通過。落ちたら報告する。`同X` が新たにマッチするようになるため、`reference-links` の既存ケースの挙動が変わっていないかを見る（Task 2 で先行詞を実装するまで、`同X` は着地先が決まらず無リンクのままになるはず）
...
```bash
git add src/core/jump/reference-pattern.ts src/core/jump/reference-pattern.test.ts
...
`const parsed = parseReference(match[0]);` の直前に、同参照の分岐を差し込む。

```ts
// 同参照は先行詞から解決する。parseReference は同X を消費して捨てるため、
// ここで先に処理する。
