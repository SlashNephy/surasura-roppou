...
## 10. 既知の制限（実装レビューで確認、いずれも fixture 受け入れ範囲外）

実装後のレビュー（最終ブランチレビュー・静的レビュー）で確認した、承認済み表記テーブルと fixture の受け入れ範囲を超える入力の挙動。いずれも参照不能時は `undefined` を返して名称検索へ graceful に落ちる（silent なデータ破壊はしない）。将来必要になれば #24 側の辞書最長一致や本パーサーの拡張で対応する。

- **空白区切りの数値マージ**: `民709 2項` は共有 `normalizeForSearch` が空白を除去して `民7092項` になり、`7092` が 1 数値として読まれる。辞書照合と正規化を揃えるための設計選択（§3）に由来する既知の副作用。
- **法令名に数字・「第」・漢数字を含む名称**: `第二種電気工事士法第3条` や `昭和四十三年法律第百号第1条` は、先頭の非数値ランで法令名を切り出す境界ヒューリスティック（§5）では正しく分離できない。辞書登録法令（行政書士頻出）はこの形を取らない。正確な切り出しには辞書最長一致が要る（#24 の関心事に近い）。
- **「同」マーカーの連結**: `同法同条第1項` のような同マーカーを 2 つ以上連ねた形は、条レベル・項レベルを各 1 回だけ消費する実装では捌けず `undefined` になる。単独の `同法` / `同条` / `同項` は対応済み。
- **辞書外法令名の原文復元**: 辞書に一致しない法令名候補は正規化後トークンを `lawNameCandidate` に載せる（§4 の「原文」との差）。漢数字のみの名称では原文と一致するため fixture は通る。全角・空白を含む辞書外名称のみ差が出るが、消費側 #24 は width・大小非依存で解決するため許容する。
