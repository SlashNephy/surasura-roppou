...
interface LawCatalogEntry {
  lawId: string;
  title: string;
  titleKana?: string; // e-Gov の law_title_kana。よみ検索に使える
  lawNumber?: string;
  lawType?: string;
...
- **ローカル照合**: `lawCatalog`（キャッシュ + 保存済み法令のシード）を線形走査し、`title` / `titleKana` / `aliases` を正規化部分一致、`lawNumber` を正規化一致で照合する。カタログは小規模なので線形で十分であり、将来大きくなれば bigram 化できる。
