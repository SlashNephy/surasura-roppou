...
describe("createPwaUpdateController", () => {
  it("publishes offline-ready and refresh-needed states from service worker callbacks", () => {
    ...
    unsubscribe();
    const callCountBeforeError = listener.mock.calls.length;
    register.mock.calls[0]?.[0].onRegisterError?.(new Error("boom"));

    expect(listener).toHaveBeenCalledTimes(callCountBeforeError);
  });

  it("publishes registration errors while listeners are active", () => {
