import { describe, expect, it } from "vitest";

import {
  readabilityHeadingTransformFixtures,
  readabilityTransformFixtures,
} from "@/test/fixtures/readability";

import { transformReadableHeadingText, transformReadableText, toArabicNumber } from "./readability";

describe("readability", () => {
  it.each(readabilityTransformFixtures)(
    "transforms fixture: $name",
    ({ expected, input, mode }) => {
      expect(transformReadableText(input, mode)).toBe(expected);
    },
  );

  it.each(readabilityHeadingTransformFixtures)(
    "transforms heading fixture: $name",
    ({ expected, input }) => {
      expect(transformReadableHeadingText(input)).toBe(expected);
    },
  );

  it.each([
    // 見出しの先頭でなくても本文と同じ規則で構造番号を変換する（issue #205）。
    // 枝番に読めない「の二つ」は漢数字のまま残す。
    ["第一章及び第二章", "第1章及び第2章"],
    ["第四章の二つの規定", "第4章の二つの規定"],
  ])("applies the body structural rule in heading transform: %s", (input, expected) => {
    expect(transformReadableHeadingText(input)).toBe(expected);
  });

  it.each([
    ["第十十章の二　第一条", "第十十章の二　第1条"],
    ["第四章の十十　第一条", "第4章の十十　第1条"],
  ])("preserves malformed heading number components: %s", (input, expected) => {
    expect(transformReadableHeadingText(input)).toBe(expected);
  });

  it.each([
    ["", ""],
    [
      "私権は、公共の福祉に適合しなければならない。",
      "私権は、公共の福祉に適合しなければならない。",
    ],
    [
      "一般、一部、同一、第三者、第一審、第一義的。",
      "一般、一部、同一、第三者、第一審、第一義的。",
    ],
    ["第一目標を達成する。", "第一目標を達成する。"],
    ["第一目的は安全である。", "第一目的は安全である。"],
    ["第一編成を発表する。", "第一編成を発表する。"],
    ["第一目安として扱う。", "第一目安として扱う。"],
  ])("keeps non-target prose unchanged: %s", (input, expected) => {
    expect(transformReadableText(input)).toBe(expected);
  });

  it.each([
    // 本文中の編・章・節・款・目への参照（issue #205）
    ["第四編（親族）の規定に従い", "第4編（親族）の規定に従い"],
    ["第一章及び第二章", "第1章及び第2章"],
    ["第一目から第三目まで", "第1目から第3目まで"],
    ["第一章又は第二章", "第1章又は第2章"],
    ["第四章の二又は第五章", "第4章の2又は第5章"],
    ["第二編第二章第八節の規定", "第2編第2章第8節の規定"],
    ["前編第八章第二節の規定", "前編第8章第2節の規定"],
    ["第五編の規定により株主", "第5編の規定により株主"],
    ["第六章から第八章までの規定", "第6章から第8章までの規定"],
    ["第三編ヲ除ク外", "第3編ヲ除ク外"],
    ["同章第七節及び第八節並びに", "同章第7節及び第8節並びに"],
    // 枝番も、続く語と紛れない位置なら変換する
    ["第四章の二で定める。", "第4章の2で定める。"],
    ["第四章の二より適用する。", "第4章の2より適用する。"],
    ["第四章の二以下", "第4章の2以下"],
    ["第四章の二以下の規定", "第4章の2以下の規定"],
    ["第四章の二の一部を改正する。", "第4章の2の一部を改正する。"],
    ["第五編第四章及び第四章の二並びに同編第五章", "第5編第4章及び第4章の2並びに同編第5章"],
  ])("transforms structural references in body text: %s", (input, expected) => {
    expect(transformReadableText(input)).toBe(expected);
  });

  it.each([
    // 枝番が数量・別の語に読める位置では、構造番号だけ変換して枝番は漢数字のまま残す
    ["第四章の二つ", "第4章の二つ"],
    ["第四章の二か所", "第4章の二か所"],
    ["第四章の二か所を改正する。", "第4章の二か所を改正する。"],
    ["第四章の二とおり", "第4章の二とおり"],
    ["第四章の二かつ第三章の三", "第4章の二かつ第3章の3"],
    ["第四章の一部を改正する。", "第4章の一部を改正する。"],
    ["第三節の二次的な効果", "第3節の二次的な効果"],
  ])("keeps an ambiguous branch number unchanged: %s", (input, expected) => {
    expect(transformReadableText(input)).toBe(expected);
  });

  it("applies all readable transforms in a stable order", () => {
    expect(transformReadableText("平成五年法律第八十八号（令和六年四月一日施行）第一条")).toBe(
      "平成5年法律第88号（令和6年4月1日施行）第1条",
    );
  });

  it.each([
    ["損害(精神的損害を含む。)", "損害（精神的損害を含む。）"],
    ["損害（精神的損害を含む。）", "損害（精神的損害を含む。）"],
  ])("normalizes half-width parentheses to full-width: %s", (input, expected) => {
    expect(transformReadableText(input)).toBe(expected);
  });

  it.each([
    ["２", "2"],
    ["２０２４", "2024"],
    ["第２項", "第2項"],
  ])("normalizes full-width digits to half-width: %s", (input, expected) => {
    expect(transformReadableText(input)).toBe(expected);
  });

  it.each([
    ["第一号の二", "第1号の2"],
    ["第十二条の二の二", "第12条の2の2"],
    ["令和元年五月一日", "令和元年5月1日"],
    ["平成元年法律第十一号", "平成元年法律第11号"],
    ["別記様式第一", "別記様式1"],
    ["別表第一の二", "別表1の2"],
    ["別記様式第一の二", "別記様式1の2"],
    ["昭和二十一年憲法", "昭和21年憲法"],
    ["大正十一年勅令第五百十八号", "大正11年勅令第518号"],
    ["平成十二年政令第三百号", "平成12年政令第300号"],
  ])("transforms common legal notation: %s", (input, expected) => {
    expect(transformReadableText(input)).toBe(expected);
  });

  it.each([
    ["一年間の期間を定める。", "1年間の期間を定める。"],
    ["三箇月以内に請求する。", "3箇月以内に請求する。"],
    ["三月以内に限る。", "3月以内に限る。"],
    ["二週間を経過したとき。", "2週間を経過したとき。"],
    ["二以上の法令に抵触する。", "2以上の法令に抵触する。"],
    ["総トン数二十トン未満の船舶", "総トン数20トン未満の船舶"],
    [
      "船荷証券の一通又は数通を交付しなければならない。",
      "船荷証券の1通又は数通を交付しなければならない。",
    ],
    [
      "二人以上の船荷証券の所持人がある場合において、その一人が運送品の引渡しを受けたとき。",
      "2人以上の船荷証券の所持人がある場合において、その1人が運送品の引渡しを受けたとき。",
    ],
    [
      "その三分の二を船舶所有者に支払い、その三分の一を船員に支払わなければならない。",
      "その3分の2を船舶所有者に支払い、その3分の1を船員に支払わなければならない。",
    ],
    ["十分の一以上の議決権", "10分の1以上の議決権"],
    ["三月を超える期間", "3月を超える期間"],
    ["十を超える議決権", "10を超える議決権"],
    // 主要法令の実データ走査で見つかった助数詞
    ["十五歳未満の者を養子とする", "15歳未満の者を養子とする"],
    ["国会の常会は、毎年一回これを召集する。", "国会の常会は、毎年1回これを召集する。"],
    ["二個以上の刑", "2個以上の刑"],
    ["拘禁刑の長期の二倍以下とする", "拘禁刑の長期の2倍以下とする"],
    ["一週間について四十時間を超えて", "1週間について40時間を超えて"],
    ["通常の賃金の計算額の二割五分以上", "通常の賃金の計算額の2割5分以上"],
    ["計算額の五割以上の率", "計算額の5割以上の率"],
    ["四親等内の親族", "4親等内の親族"],
    ["二棟の建物", "2棟の建物"],
    ["十労働日の有給休暇", "10労働日の有給休暇"],
    ["設立時発行株式を一株以上引き受ける", "設立時発行株式を1株以上引き受ける"],
    ["一単元の株式とする", "1単元の株式とする"],
    ["その日から起算して六月をこえない期間", "その日から起算して6月をこえない期間"],
    ["その日から起算して三月を経過した日", "その日から起算して3月を経過した日"],
    ["その日から起算して六月間", "その日から起算して6月間"],
    // 「二以上」だけが変換されて「一又は2以上」と不揃いになるのを防ぐ
    ["一又は二以上の株式", "1又は2以上の株式"],
    // 金額は万・億を単位語として残す
    ["罰金は、一万円以上とする", "罰金は、1万円以上とする"],
    ["五十万円以下の罰金に処する", "50万円以下の罰金に処する"],
    ["出資の総額が三億円", "出資の総額が3億円"],
    ["科料は、千円以上一万円未満とする", "科料は、1000円以上1万円未満とする"],
    ["五円未満の端数", "5円未満の端数"],
    ["二百億円以上であるとき", "200億円以上であるとき"],
    // 月名は範囲表現に限って変換する
    ["一月から三月まで、四月から六月まで", "1月から3月まで、4月から6月まで"],
    ["一月から前々年の十二月までの各月", "1月から前々年の12月までの各月"],
    ["定款で一単位の株式の数を定める", "定款で1単位の株式の数を定める"],
    ["労働者を二組以上に分ける", "労働者を2組以上に分ける"],
  ])("transforms quantity notation: %s", (input, expected) => {
    expect(transformReadableText(input)).toBe(expected);
  });

  it.each([
    ["十分な注意を払う。", "十分な注意を払う。"],
    ["一通りの手続を経る。", "一通りの手続を経る。"],
    ["同一人が行う行為", "同一人が行う行為"],
    ["同一年度内に限る。", "同一年度内に限る。"],
    ["一部の規定は適用しない。", "一部の規定は適用しない。"],
    ["一般に公表する。", "一般に公表する。"],
    ["その分野の第一人者による監修", "その分野の第一人者による監修"],
    // 法律書の慣行に合わせ、条項の相対参照は漢数字のまま残す。
    ["前三条の規定は、前二項の場合に準用する。", "前三条の規定は、前二項の場合に準用する。"],
    ["前二号に掲げるもの", "前二号に掲げるもの"],
    ["十分に理解する。", "十分に理解する。"],
    ["一時的な通信障害", "一時的な通信障害"],
    ["四半期における労働者", "四半期における労働者"],
    ["第一順位の先取特権", "第一順位の先取特権"],
    ["第三取得者による弁済", "第三取得者による弁済"],
    ["一種又は数種の営業", "一種又は数種の営業"],
  ])("keeps quantity-like prose unchanged: %s", (input, expected) => {
    expect(transformReadableText(input)).toBe(expected);
  });

  it("keeps era dates intact while transforming quantities in the same sentence", () => {
    expect(transformReadableText("令和六年四月一日から起算して三箇月以内に、第一条の規定")).toBe(
      "令和6年4月1日から起算して3箇月以内に、第1条の規定",
    );
  });

  it("keeps a branch article number from being read as a fraction", () => {
    expect(transformReadableText("第三十四条の二の規定")).toBe("第34条の2の規定");
  });

  it.each([
    ["一", 1],
    ["十", 10],
    ["十一", 11],
    ["十二", 12],
    ["二十", 20],
    ["八十八", 88],
    ["百一", 101],
    ["九百九十九", 999],
  ])("converts kanji number %s", (input, expected) => {
    expect(toArabicNumber(input)).toBe(expected);
  });

  it.each(["", "〇", "零", "壱", "一2"])("returns undefined for unsupported number %s", (input) => {
    expect(toArabicNumber(input)).toBeUndefined();
  });

  it.each(["十十", "九千九千", "十百", "百百"])(
    "returns undefined for malformed kanji number %s",
    (input) => {
      expect(toArabicNumber(input)).toBeUndefined();
    },
  );

  it.each([
    ["第十十条", "第十十条"],
    ["第十百号", "第十百号"],
    ["第十十章", "第十十章"],
    ["第百百節", "第百百節"],
  ])("keeps malformed legal number unchanged: %s", (input, expected) => {
    expect(transformReadableText(input)).toBe(expected);
  });

  it("converts a valid article base while preserving its malformed branch number", () => {
    expect(transformReadableText("第四条の十十")).toBe("第4条の十十");
  });
});
